...
@section('head-tag')
<title>ویرایش گارانتی {{$product->name}}</title>
...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb text-center">
        ...
        <li class="breadcrumb-item font-size-12"><a
                href="{{route('admin.market.product-guarantee.index', $product->id)}}">مدیریت گارانتی های {{$product->name}}</a>
        </li>
        <li class="breadcrumb-item active font-size-12" aria-current="page">ویرایش گارانتی {{$product->name}}</li>
    </ol>
</nav>
<section class="row">
    ...
    <section class="col-12">
        <section class="main-body-container">
            <section class="main-body-container-header">
                <h4>ویرایش گارانتی {{$product->name}}</h4>
            </section>
            <section class="d-flex justify-content-between align-items-center my-4">
                <a class="btn btn-dark btn-sm"
                    href="{{route('admin.market.product-guarantee.index', $product->id)}}">بازگشت</a>

            </section>
            <section>
                <form action="{{route('admin.market.product-guarantee.update', ['product' => $product->id, 'guarantee' => $guarantee->id])}}" method="post">
                    <section class="row">
                        @csrf
                        @method('put') 
                        <section class="col-12 col-md-5 py-2">
                            <div class="form-group">
                                ...
                                <input class="form-control form-control-sm" type="text"
                                    value="{{old('name', $guarantee->name)}}" name="name" id="name">
                            </div>
                            @error('name')
                        ...
                        <section class="col-12 col-md-5 py-2">
                            <div class="form-group">
                                ...
                                <input class="form-control form-control-sm" type="text"
                                    value="{{old('price_increase', $guarantee->price_increase)}}" name="price_increase"
                                    id="price_increase">
                            </div>
