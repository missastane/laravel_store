    ...
    <section class="col-12">
        <section class="main-body-container">
            <section class="main-body-container-header">
                <h4>بخش برند محصولات</h4>
            </section>
            <section class="d-flex justify-content-between align-items-center my-4">
                <a class="btn btn-primary btn-sm" href="{{route('admin.market.brand.create')}}">ایجاد برند جدید</a>
                <form method="GET" action="{{route('admin.market.brand.search')}}" class="max-width-16-rem">
                ...
                <table class="table table-striped table-info text-center">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><img class="notification-img" src="{{asset($brand->logo['indexArray'][$brand->logo['currentImage']])}}" alt=""></td>
                            <td>{{$brand->tags}}</td>
                            <td>
                                    @if ($brand->status == 2)
                                        <span><i class="text-danger fa fa-toggle-off pointer {{$brand->id}} status"
                                                onclick="changeActivation({{$brand->id}}, '.status')"
                                                data-url="{{route('admin.market.brand.status', $brand->id)}}"
                                                id="{{$brand->id}}"></i><b class="info">غیرفعال</b></span>
                                    @else
                                        <span><i class="text-success fa fa-toggle-on pointer {{$brand->id}} status"
                                                onclick="changeActivation({{$brand->id}}, '.status')"
                                                data-url="{{route('admin.market.brand.status', $brand->id)}}"
                                                id="{{$brand->id}}"></i> <b class="info">فعال</b></span>
                                    @endif
                                </td>
                            <td class="width-16-rem text-left">
                                <a class="btn btn-sm btn-warning" href="{{route('admin.market.brand.edit', $brand->id)}}"><i class="fa fa-pencil-alt ml-1"></i>ویرایش</a>
