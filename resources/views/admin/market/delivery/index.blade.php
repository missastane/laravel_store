    ...
    <section class="col-12">
        <section class="main-body-container">
            <section class="main-body-container-header">
                <h4>روش های ارسال محصولات</h4>
            </section>
            <section class="d-flex justify-content-between align-items-center my-4">
                <a class="btn btn-primary btn-sm" href="{{route('admin.market.delivery.create')}}">ایجاد روش جدید</a>
                <form method="GET" action="{{route('admin.market.delivery.search')}}" class="max-width-16-rem">
                ...
                <table class="table table-striped table-info text-center">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                    @if ($delivery_method->status == 2)
                                        <span><i class="text-danger fa fa-toggle-off pointer"
                                                onclick="changeStatus({{$delivery_method->id}})"
                                                data-url="{{route('admin.market.delivery.status', $delivery_method->id)}}"
                                                id="{{$delivery_method->id}}"></i><b class="info">غیرفعال</b></span>
                                    @else
                                        <span><i class="text-success fa fa-toggle-on pointer"
                                                onclick="changeStatus({{$delivery_method->id}})"
                                                data-url="{{route('admin.market.delivery.status', $delivery_method->id)}}"
                                                id="{{$delivery_method->id}}"></i> <b class="info">فعال</b></span>
                                    @endif
                                </td>
                            <td class="width-16-rem text-left">
                                <a class="btn btn-sm btn-warning" href="{{route('admin.market.delivery.edit', $delivery_method->id)}}"><i class="fa fa-pencil-alt ml-1"></i>ویرایش</a>
                                <form class="d-inline"
