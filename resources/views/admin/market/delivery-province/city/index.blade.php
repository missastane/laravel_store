...
@section('head-tag')
<title>شهرهای استان {{$province->name}}</title>
@endsection
    ...
    <section class="col-12">
        <section class="main-body-container">
            <section class="main-body-container-header">
                <h4>شهرهای استان {{$province->name}}</h4>
            </section>
            <section class="d-flex justify-content-between align-items-center my-4">
                <section>
                <a class="btn btn-dark btn-sm" href="{{route('admin.market.delivery-province.index')}}">بازگشت</a>
                <a class="btn btn-primary btn-sm" href="{{route('admin.market.delivery-city.create', $province)}}">ایجاد شهر جدید</a>
                </section>
                <form method="GET" action="{{route('admin.market.delivery-city.search', $province)}}" class="max-width-16-rem">
                ...
                <table class="table table-striped table-info text-center">
                    ...
                    @endphp
                        ...
                        <tr>
                            ...
                            <a class="btn btn-sm btn-warning" href="{{route('admin.market.delivery-city.edit', $city->id)}}"><i class="fa fa-pencil-alt ml-1"></i>ویرایش</a>
                                <form class="d-inline"
                                        action="{{route('admin.market.delivery-city.destroy', $city->id)}}"
                                        method="post">
                                        @csrf
                                        {{method_field('delete')}}
