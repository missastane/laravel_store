    ...
    <section class="col-12">
        <section class="main-body-container">
            ...
            <section class="d-flex justify-content-between align-items-center my-4">
                <a class="btn btn-primary btn-sm" href="{{route('admin.market.discount.amazingSale.create')}}">افزودن
                    کالا به لیست فروش شگفت انگیز</a>
                <form method="GET" action="{{route('admin.market.discount.amazingSale.search')}}" class="max-width-16-rem">
                ...
                <table class="table table-striped table-info text-center">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                    @if ($amazingSale->status == 2)
                                        <span><i class="text-danger fa fa-toggle-off pointer"
                                                onclick="changeStatus({{$amazingSale->id}})"
                                                data-url="{{route('admin.market.discount.amazingSale-status', $amazingSale->id)}}"
                                                id="{{$amazingSale->id}}"></i><b class="info">غیرفعال</b></span>
                                    @else
                                        <span><i class="text-success fa fa-toggle-on pointer"
                                                onclick="changeStatus({{$amazingSale->id}})"
                                                data-url="{{route('admin.market.discount.amazingSale-status', $amazingSale->id)}}"
                                                id="{{$amazingSale->id}}"></i> <b class="info">فعال</b></span>
                                    @endif
                                </td>
                            <td class="width-16-rem text-left">
                                <a class="btn btn-sm btn-warning" href="{{route('admin.market.discount.amazingSale.edit', $amazingSale->id)}}"><i
