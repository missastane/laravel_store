...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb text-center">
        ...
        <li class="breadcrumb-item active font-size-12" aria-current="page">اطلاعیه پیامکی</li>
    </ol>
</nav>
<section class="row">
    <section class="col-12">
        <section class="main-body-container">
            <section class="main-body-container-header">
                <h4>بخش اطلاعیه پیامکی</h4>
            </section>
            <section class="d-flex justify-content-between align-items-center my-4">
                <a class="btn btn-primary btn-sm" href="{{route('admin.notify.sms.create')}}">ایجاد اطلاعیه پیامکی جدید</a>
                <form method="GET" action="{{route('admin.notify.sms.search')}}" class="max-width-16-rem">
                ...
                <table class="table table-striped table-info text-center">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{$msg->title}}</td>
                            <td>{{$msg->body}}</td>
                            <td>{{jalalidate($msg->published_at)}}</td>
                            <td>
                                    @if ($msg->status == 2)
                                        <span><i class="text-danger fa fa-toggle-off pointer"
                                                onclick="changeStatus({{$msg->id}})"
                                                data-url="{{route('admin.notify.sms.status', $msg->id)}}"
                                                id="{{$msg->id}}"></i><b class="info">غیرفعال</b></span>
                                    @else
                                        <span><i class="text-success fa fa-toggle-on pointer"
                                                onclick="changeStatus({{$msg->id}})"
                                                data-url="{{route('admin.notify.sms.status', $msg->id)}}"
                                                id="{{$msg->id}}"></i> <b class="info">فعال</b></span>
                                    @endif
                                </td>
                            <td class="width-16-rem text-left">
                                <a class="btn btn-sm btn-warning" href="{{route('admin.notify.sms.edit', $msg->id)}}"><i class="fa fa-pencil-alt ml-1"></i>ویرایش</a>
                                <form class="d-inline"
                                        action="{{route('admin.notify.sms.destroy', $msg->id)}}"
                                        method="post">
                                        @csrf
                                        {{method_field('delete')}}
                                    ...
                                    <a class="btn btn-sm btn-primary" href="{{route('admin.notify.sms.send', $msg)}}">ارسال</a>
