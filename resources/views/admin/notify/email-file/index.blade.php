...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb text-center">
        ...
        <li class="breadcrumb-item active font-size-12" aria-current="page">فایل های اطلاعیه ایمیلی</li>
    </ol>
</nav>
<section class="row">
    <section class="col-12">
        <section class="main-body-container">
            ...
            <section class="main-body-container-header">
                <h4>بخش فایل های اطلاعیه ایمیلی</h4>
                <a class="btn btn-sm btn-secondary mt-2" href="{{route('admin.notify.email.show', $email->id)}}">بازگشت</a>

            </section>
            <section class="d-flex justify-content-between align-items-center my-4">
                
                <a class="btn btn-primary btn-sm" href="{{route('admin.notify.email-file.create', $email->id)}}">ایجاد فایل اطلاعیه ایمیلی
                    </a>
                    
                <form method="GET" action="{{route('admin.notify.email-file.search', $email)}}" class="max-width-16-rem d-flex flex-row-reverse justify-content-between">
                    <input type="text" class="form-control form-control-sm form-text" name="search" id="" placeholder="جستجوی فرمت فایل">
            ...
            <section class="table-responsive">
                <table class="table table-striped table-info text-center">
                    ...
                    <tbody>
                        @php
                        if (session('emailFiles')) {

                            $emailFiles = request()->session()->get('emailFiles');
                            request()->session()->forget('emailFiles');
                        } else {
                            $emailFiles = $email->files()->orderBy('file_size', 'desc')->get();
                        }
                    @endphp
                        @foreach ($emailFiles as $key => $file)
                            <tr>
                                <th>{{$key+=1}}</th>
                                <td>{{$email->subject}}</td>
                                ...
                                <td>
                                    @if ($file->status == 2)
                                        <span><i class="text-danger fa fa-toggle-off pointer"
                                                onclick="changeStatus({{$file->id}})"
                                                data-url="{{route('admin.notify.email-file.status', $file->id)}}"
                                                id="{{$file->id}}"></i><b class="info">غیرفعال</b></span>
                                    @else
                                        <span><i class="text-success fa fa-toggle-on pointer"
                                                onclick="changeStatus({{$file->id}})"
                                                data-url="{{route('admin.notify.email-file.status', $file->id)}}"
                                                id="{{$file->id}}"></i> <b class="info">فعال</b></span>
                                    @endif
                                </td>
                                <td class="width-16-rem text-left">
                                
                                <a class="btn btn-sm btn-info" target="_blank" href="{{route('admin.notify.email-file.openFile', $file->id)}}"><i
                                class="fa fa-download ml-1"></i>باز کردن</a>
                                    <a class="btn btn-sm btn-warning" href="{{route('admin.notify.email-file.edit', $file->id)}}"><i
