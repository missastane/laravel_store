...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb text-center">
        ...
        <li class="breadcrumb-item active font-size-12" aria-current="page">اولویت بندی</li>
    </ol>
</nav>
<section class="row">
    <section class="col-12">
        <section class="main-body-container">
            <section class="main-body-container-header">
                <h4>بخش اولویت بندی تیکت ها</h4>
            </section>
            @include('admin.alerts.alert-section.success')
            <section class="d-flex justify-content-between align-items-center my-4">
                <a class="btn btn-primary btn-sm" href="{{route('admin.ticket.priority.create')}}">ایجاد اولویت جدید</a>

                <form method="GET" action="{{route('admin.ticket.priority.search')}}" class="max-width-16-rem">
                    ...
                    <tbody>
                        @foreach ($ticketPriorities as $key => $ticketPriority)
                            <tr>
                                ...
                                <td>
                                    @if ($ticketPriority->status == 2)
                                        <span><i class="text-danger fa fa-toggle-off pointer"
                                                onclick="changeStatus({{$ticketPriority->id}})"
                                                data-url="{{route('admin.ticket.priority.status', $ticketPriority->id)}}"
                                                id="{{$ticketPriority->id}}"></i><b class="info">غیرفعال</b></span>
                                    @else
                                        <span><i class="text-success fa fa-toggle-on pointer"
                                                onclick="changeStatus({{$ticketPriority->id}})"
                                                data-url="{{route('admin.ticket.priority.status', $ticketPriority->id)}}"
                                                id="{{$ticketPriority->id}}"></i> <b class="info">فعال</b></span>
                                    @endif
                                </td>
                                <td class="width-16-rem text-left">
                                    <a class="btn btn-sm btn-warning"
                                        href="{{route('admin.ticket.priority.edit', $ticketPriority->id)}}"><i
