    ...
    <section class="col-12">
        <section class="main-body-container">
            <section class="main-body-container-header">
                <h4>بخش دسته بندی تیکت ها</h4>
            </section>
            @include('admin.alerts.alert-section.success')
            <section class="d-flex justify-content-between align-items-center my-4">
                <a class="btn btn-primary btn-sm" href="{{route('admin.ticket.category.create')}}">ایجاد دسته جدید</a>

                <form method="GET" action="{{route('admin.ticket.category.search')}}" class="max-width-16-rem">
                    ...
                    <tbody>
                        @foreach ($ticketCategories as $key => $ticketCategory)
                            <tr>
                                ...
                                <td>
                                    @if ($ticketCategory->status == 2)
                                        <span><i class="text-danger fa fa-toggle-off pointer"
                                                onclick="changeStatus({{$ticketCategory->id}})"
                                                data-url="{{route('admin.ticket.category.status', $ticketCategory->id)}}"
                                                id="{{$ticketCategory->id}}"></i><b class="info">غیرفعال</b></span>
                                    @else
                                        <span><i class="text-success fa fa-toggle-on pointer"
                                                onclick="changeStatus({{$ticketCategory->id}})"
                                                data-url="{{route('admin.ticket.category.status', $ticketCategory->id)}}"
                                                id="{{$ticketCategory->id}}"></i> <b class="info">فعال</b></span>
                                    @endif
                                </td>
                                <td class="width-16-rem text-left">
                                    <a class="btn btn-sm btn-warning"
                                        href="{{route('admin.ticket.category.edit', $ticketCategory->id)}}"><i
