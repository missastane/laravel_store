...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb text-center">
        ...
        <li class="breadcrumb-item active font-size-12" aria-current="page">سؤالات متداول</li>
    </ol>
</nav>
<section class="row">
    <section class="col-12">
        <section class="main-body-container">
            <section class="main-body-container-header">
                <h4>بخش سؤالات متداول</h4>
            </section>
            <section class="d-flex justify-content-between align-items-center my-4">
                <a class="btn btn-primary btn-sm" href="{{route('admin.content.faq.create')}}">ایجاد سؤال و جواب جدید</a>
                <form method="GET" action="{{route('admin.content.faq.search')}}" class="max-width-16-rem">
                ...
                <table class="table table-striped table-info text-center">
                    ...
                    @foreach ($faqs as $key=> $faq)
                        <tr>
                            <th>{{$key += 1}}</th>
                            <td>{{$faq->question}}</td>
                            <td>{{Str::limit(strip_tags($faq->answer),15)}}...</td>
                            <td>{{$faq->tags}}</td>
                            <td>
                                    @if ($faq->status == 2)
                                        <span><i class="text-danger fa fa-toggle-off pointer"
                                                ...
                                                id="{{$faq->id}}"></i><b class="info">غیرفعال</b></span>
                                    @else
                                        <span><i class="text-success fa fa-toggle-on pointer"
                                                onclick="changeStatus({{$faq->id}})"
                                                data-url="{{route('admin.content.faq.status', $faq->id)}}"
                                                id="{{$faq->id}}"></i> <b class="info">فعال</b></span>
                                    @endif
                                </td>
                                <td class="width-16-rem text-left">
                                <a class="btn btn-sm btn-info"
                                        href="{{route('admin.content.faq.show', $faq->id)}}"><i
                                            class="fa fa-eye ml-1"></i>جزئیات</a>
                                    <a class="btn btn-sm btn-warning"
                                        href="{{route('admin.content.faq.edit', $faq->id)}}"><i
