    ...
    <section class="col-12">
        <section class="main-body-container">
            ...
            <section class="d-flex justify-content-between align-items-center my-4">
                <a href="{{route('admin.content.post.create')}}"  class="btn btn-primary btn-sm">افزودن پست جدید</a>
                <form method="GET" action="{{route('admin.content.post.search')}}" class="max-width-16-rem">
                ...
                <table class="table table-striped table-info text-center vertical-middle">
                    ...
                    <tbody>
                        ...
                        <tr class="vertical-middle">
                          ...
                          <td><img  class="notification-img" 
                          src="{{asset($post->image['indexArray'][$post->image['currentImage']])}}"
                            alt=""></td>
                            <td>
                                    @if ($post->status == 2)
                                        <span><i class="text-danger fa fa-toggle-off pointer {{$post->id}} status"
                                                onclick="changeActivation({{$post->id}}, '.status')"
                                                data-url="{{route('admin.content.post.status', $post->id)}}"
                                                id="{{$post->id}}"></i><b class="info">غیرفعال</b></span>
                                    @else
                                        <span><i class="text-success fa fa-toggle-on pointer {{$post->id}} status"
                                                onclick="changeActivation({{$post->id}}, '.status')"
                                                data-url="{{route('admin.content.post.status', $post->id)}}"
                                                id="{{$post->id}}"></i> <b class="info">فعال</b></span>
                                    @endif
                                </td>
                                <td>
                                    @if ($post->commentable == 2)
                                        <span><i class="text-danger fa fa-toggle-off pointer {{$post->id}} commentable"
                                                onclick="changeActivation({{$post->id}}, '.commentable')"
                                                data-url="{{route('admin.content.post.commentable', $post->id)}}"
                                                id="{{$post->id}}"></i><b class="info">غیرفعال</b></span>
                                    @else
                                        <span><i class="text-success fa fa-toggle-on pointer {{$post->id}} commentable"
                                                onclick="changeActivation({{$post->id}}, '.commentable')"
                                                data-url="{{route('admin.content.post.commentable', $post->id)}}"
                                                id="{{$post->id}} "></i> <b class="info">فعال</b></span>
                                    @endif
                            ...
                            <a class="btn btn-sm btn-warning" href="{{route('admin.content.post.edit', $post->id)}}"><i class="fa fa-pencil-alt ml-1"></i>ویرایش</a>
