            ...
            <form action="{{route('admin.content.menu.store')}}" method="post">
                <section class="row">
                   ...
                   <select class="form-control form-control-sm" name="parent_id" id="parent_id">
                    <option value="" @if(old('parent_id') == null)
                    selected
                    @endif>-</option>
                    @foreach ($parentMenus as $parentMenu)
                    <option value="{{$parentMenu->id}}" @if(old('parent_id') == $parentMenu->id)
                    selected
                    @endif
                    ...
                    <div class="form-group">
                        ...
                        <input title="نمونه آدرس معتبر : https://www.google.com/everything یا http://google.com/everything" class="form-control form-control-sm" value="{{old('url')}}" type="text" name="url" id="url">
                    </div>
                    @error('url')
                                <span class="alert_required text-danger" role="alert"><strong>{{$message}}</strong></span>
                            @enderror
